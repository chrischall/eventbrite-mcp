 ...
 *
 * The read tools in this server hand back Eventbrite's payload close to
 ...
 * honestly say which of Eventbrite's fields matter and which are noise.
 *
...
const NOTE =
  ...
  'No field projection: this server has no verified record of which Eventbrite fields matter, and inventing ' +
  'one would risk dropping a field a caller needs.';

/** The `view` parameter every read tool in this server takes. */
export const viewArg = (): ReturnType<typeof viewParam> => viewParam(EB_VIEWS, { note: NOTE });
